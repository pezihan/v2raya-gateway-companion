...
### 3.4 域名输入智能建议与防重复查重机制

无论是在客户端界面手动添加域名，还是通过浏览器扩展一键添加，均严格遵循以下输入建议与校验逻辑：

1. **输入清洗与根域名自动建议 (Input Normalization & Smart Suggestion)**：
   - 用户输入或粘贴任何形式的内容（如完整 URL `https://mail.google.com/path` 或带端口 `sub.domain.com:8080`），前端自动清洗提取出纯 FQDN 域名。
   - **实时建议气泡**：若识别到输入的是多级子域名（如 `www.notion.so`），输入框下方即时弹出建议栏：
     - 💡 *“检测到多级域名，建议添加根域名 `*.notion.so`（自动包含所有子域名）”*
     - **交互动作**：
       - 点击 **[ 采用建议 ]**：输入框自动切换为根域名 `notion.so`，规则类型设为 `domain_suffix`。
       - 点击 **[ 保留原样 ]** / 不点：保留原本的 `www.notion.so`（以精确 `domain` 规则添加）。

2. **多层级防重复与冗余校验 (Deduplication & Coverage Conflict Prevention)**：
   - **完全重复拒绝**：提交时检索 Isar 数据库，若完全相同的域名与匹配规则已存在，直接拦截并报错提示：`“该域名规则已存在，请勿重复添加！”`。
   - **已被根规则覆盖校验**：若用户尝试添加 `mail.google.com`，而规则库中已经存在 `google.com` (domain_suffix)，系统拦截并友好提示：`“该域名已被现有的根域名规则 *.google.com 覆盖，无需重复添加”`。
   - **子规则智能收敛合并**：若规则库中原本已单独存在 `www.example.com`，用户后续决定添加根域名 `example.com` (domain_suffix)，系统弹出提示：`“检测到已有 1 条相关的子域名规则，添加根域名将自动收敛并清理历史子规则，是否合并？”`，保证规则库始终精炼整洁。
